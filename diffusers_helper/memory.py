...
def move_model_to_device_with_memory_preservation(model, target_device, preserved_memory_gb=0, stream = None):
    ...
    with torch.cuda.stream(stream) if stream else contextlib.nullcontext():
        ...
        # Synchronize the stream after moving the models
        if stream:
            stream.synchronize()
        return
...
def offload_model_from_device_for_memory_preservation(model, target_device, preserved_memory_gb=0, stream = None):
    ...
    # Synchronize the stream after offloading
    if stream:
        stream.synchronize()
    return

# Adding stream argument
def unload_complete_models(*args, stream = None):
    with torch.cuda.stream(stream) if stream else contextlib.nullcontext():
...
def load_model_as_complete(model, target_device, unload=True, stream=None):
    if unload:
        # pass the stream argument
        unload_complete_models(stream=stream)
    # Use the provided stream context
    with torch.cuda.stream(stream) if stream else contextlib.nullcontext():
        model.to(device=target_device)
